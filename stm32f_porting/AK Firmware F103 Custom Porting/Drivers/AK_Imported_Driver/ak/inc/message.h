	...
	extern "C"
	{
		...
		// Khai báo directive quản lý kích thước của các pool tin nhắn
		...
		// Khai báo directive quản lý kích thước của PDU động trong tin nhắn động
			...
			/**
			 * Ghi chú:
			 * PDU là đơn vị dữ liệu truyền tải trong tin nhắn động,
			 * có kích thước cố định được xác định bởi AK_DYNAMIC_PDU_SIZE.
			 */
		#endif

		// Khai báo các hằng số và macro liên quan đến tin nhắn
		...
		// Trả về số lượng tham chiếu của tin nhắn, giúp theo dõi số lượng tác vụ đang sử dụng tin nhắn để quản lý bộ nhớ hiệu quả
		#define get_msg_ref_count(x)		((((ak_msg_t*)x)->ref_count) & AK_MSG_REF_COUNT_MASK)
		// Quản lý việc đặt lại số lượng tham chiếu của tin nhắn về 0, giúp chuẩn bị tin nhắn để sử dụng lại hoặc giải phóng khi không còn cần thiết nữa
		#define reset_msg_ref_count(x)		((((ak_msg_t*)x)->ref_count) = \
			(((ak_msg_t*)x)->ref_count) & (~AK_MSG_REF_COUNT_MASK))
		// Trả về loại của tin nhắn, giúp xác định loại tin nhắn (thuần túy, thông thường, động) để xử lý phù hợp
		#define get_msg_type(x)				((((ak_msg_t*)x)->ref_count) & AK_MSG_TYPE_MASK)

		// Khai báo cấu trúc quản lý debug của tin nhắn
		typedef struct {
			uint32_t	start_post; // Quản lý thời gian bắt đầu đăng tin nhắn, giúp theo dõi thời gian đăng tin nhắn để phân tích hiệu suất và debug
			uint32_t	start_exe;  // Quản lý thời gian bắt đầu thực thi tin nhắn, giúp theo dõi thời gian thực thi của tin nhắn để phân tích hiệu suất và debug
		...
		// Khai báo cấu trúc quản lý tin nhắn
		typedef struct ak_msg_t {
			// Quản lý con trỏ đến tin nhắn tiếp theo trong danh sách liên kết
			struct ak_msg_t*	next; // Quản lý con trỏ đến tin nhắn tiếp theo trong danh sách liên kết

			// Quản lý debug handler
			#if (AK_TASK_DEBUG == AK_ENABLE) 
				dbg_handler_t		dbg_handler; // Quản lý thông tin debug của tin nhắn, giúp theo dõi thời gian đăng và thực thi của tin nhắn để phân tích hiệu suất và debug
			#endif
			...
			uint8_t				src_task_id; 	// Quản lý ID của tác vụ nguồn gửi tin nhắn
			...
			uint8_t				if_src_task_id; // Quản lý ID của tác vụ nguồn gửi tin nhắn qua giao diện bên ngoài
		...
		typedef struct {
			 ...
			 * thông tin đơn giản giữa các tác vụ mà không cần dữ liệu đi kèm, 
		...
		typedef struct {
			 ...
			 * hoặc không xác định trước kích thước của dữ liệu giữa các tác vụ.
			 */
		...
		extern void msg_force_free(ak_msg_t* msg);  	// Quản lý việc giải phóng tin nhắn mà không kiểm tra số lượng tham chiếu, sử dụng khi cần thiết để giải phóng ngay lập tức mà không quan tâm đến số lượng tham chiếu
		...
		// Khai báo hàm quản lý debug của tin nhắn
